...
- **验收标准**：
  ...
  - [ ] **给定** 单次爬取超时 1200 秒 **当** 爬虫超时 **则** 记录错误日志并标记该股票本次爬取失败，不阻塞其他股票
...
| 全量爬取时间 | ≤120 分钟 | 60 只自选股一次完整爬取耗时（单股 4-6 分钟） |
...
| 爬取失败重试 | 瞬时失败（浏览器握手等）立即重试最多 2 次；超时/全失败则等下一次调度 |
...
|----------|----------|----------|--------------|
| 爬虫超时 | 单次爬取 >1200 秒 | 记录错误日志，标记失败，跳过该股票 | 告警中显示"XX 股票爬取超时" |
...
def crawl_stock(stock_code: str, timeout: int = 1200) -> dict:
    """
    调用 xueqiu-analyzer 爬取单只股票数据
    
    参数：
        stock_code (str): 股票代码，如 "SH600519"
        timeout (int): 超时秒数，默认 600（V3.1 爬虫单股 4-6 分钟）
...
## 8. xueqiu-analyzer 依赖升级说明

### 8.1 当前依赖版本

- **xueqiu-analyzer v3.1.0** — 时间维度停止、四分类模型（讨论/专栏/资讯/公告）、JS 结构化提取
- **调用入口**: `crawler.py::_crawl_with_timeout()` → `XueqiuCrawler.crawl(max_pages=50, max_articles=20)`
- **超时配置**: 默认 1200s/股（`etc/config.json` → `crawler.timeout_seconds`），单股实际 2-6 分钟

### 8.2 V3.1 升级影响

| 变化项 | 旧 (V2.x) | 新 (V3.1) | 对 monitor 影响 |
|--------|-----------|-----------|----------------|
| **停止策略** | 固定 max_pages=3 | 时间维度（"昨天"触发停止） | 单股帖数从 ~30 升至 50-500，爬取时间 4-6 min |
| **专栏文章** | 混在 discussions 中 | 独立 `articles[]` 列表，自动分离 | `crawler.py` 已支持遍历 `crawl_result.articles` ✅ |
| **解析方式** | `inner_text() + regex` | `item.evaluate(JS)` 结构化 | Author 更干净，`_clean_author()` 仍作兜底 |
| **详情富化** | 无 | 专栏全文 ≤5000字 + 评论列表 | `max_articles=20` 控制富化条数 |
| **评论数据** | 仅 comment_count | 新增 `comments: list[str]` + `like_count` | 可用 but 未接入 |
| **翻页机制** | 搜索"更多"按钮 | 点击「下一页」链接 | 与 monitor 无关（内部实现） |

### 8.3 monitor 适配要点

1. **超时提升**: `crawl_single_stock(timeout=1200)` 默认值 + `etc/config.json` 配置
2. **翻页参数**: `_crawl_with_timeout` 传 `max_pages=50`（兜底）+ `max_articles=20`（富化）
3. **四分类兼容**: `crawl_result.discussions/articles/news/notices` 均已遍历 → `posts_data` 正确
4. **帖数基准**: 升级后首日帖数会显著增长，冷启动期 Z-score 偏高属正常，2-3天回归
5. **不需要改动**: `sentiment.py`、`detector.py`、`filter.py`、`notifier.py` 均无需修改

### 8.4 性能预估

| 场景 | 旧 (V2, max_pages=3) | 新 (V3.1, max_pages=50) |
|------|----------------------|-------------------------|
| 安静股票 | 30 帖 / 20s | 30 帖 / 20s（时间停止触达快） |
| 活跃股票 | 30 帖 / 30s | 400 帖 / 5 min |
| 60 股全量 (sequential) | ~30 min | ~120 min |
| 60 股全量 (concurrency=3) | ~10 min | ~40 min |