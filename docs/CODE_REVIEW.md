...
## 手跑日报（2026-09-19 22:26 真实数据）后追加发现

| # | 问题 | 处置 |
|---|------|------|
| R3-7 | 0 帖股票（特斯拉）正文不以 `（无新增量）` 开头 → 被归入"其他今日有增量的股票"完整小节，标题与内容自相矛盾；尾注仍写"全部 41 只均经 LLM 分析"，实际该股被跳过 | 无数据分支正文改带前缀归入紧凑列表 + `analyzed` 标记，尾注按实际分析数标注 |
| R3-8 | 报告 H1 无日期（`# 📊 自选股舆情日报`）→ IMA 笔记标题不带日期，笔记本里同年同日多篇无法区分（IMA 用 H1 做标题，`publish_ima.py` 的 title 参数并未传给 API） | H1 改为 `# 📊 自选股舆情日报 YYYY-MM-DD` |

| R3-9 | **分档口径错**: `classify_stock_tier` 的"当日互动总量"取自过滤后的帖集 (已剔除回复/资讯), 而高互动帖恰恰常是回复与资讯 → `deep_engagement` 通道近乎失效。2026-09-20 实测: 并集口径 15 只过 100 线, 过滤集口径只有 2 只 → 深读塌缩成 6/41, 其余 25 只走"标准"两小节模板 (用户反馈"前半详细、后半省略") | 互动量改按当日并集算 (与温度计同源); 另加材料量通道 `tier.deep_material_chars` (正文字数 ≥1 万 → 深读, 覆盖"没人点赞但材料很足"的票)。同数据重跑 09-20: 深读 6 → 21, 报告 31.4k → 51.8k 字符 |

| R3-10 | 用户决定: 取消"标准"档, **全部深读** (有帖即深读, 无帖才进平稳紧凑行)。同数据重跑 2026-09-20 实测: 深读 6 → 41 只, 报告 31.4k → 63.3k 字符, 41 次 LLM 调用 3 分 03 秒无重试; 极薄素材的 7 只按深读模板新增的"素材不足例外"只输出 2 节 (330-566 字), 未注水 | `tier.deep_all=true` (可回滚); 旧分档规则在 deep_all=false 时保留 |
